$(function () {
	...
	$('.refresh-captcha').on('click', function () {
		...
		$.ajax({
			url: BASE_URL + 'captcha/reload/' + capparent.parent('.captcha-refresh').attr('data-captcha-time'),
			dataType: 'JSON',
		})
		.done(function (res) {
			$('.captcha-image').find('.box-image').html(res.image);
			capparent.parent('.captcha-refresh').attr('data-captcha-time', res.captcha.time);
		})
		.fail(function () {
