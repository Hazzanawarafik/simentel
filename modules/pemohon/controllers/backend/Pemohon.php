...
class Pemohon extends Admin	{
	...
	*/
	public function add_save() {
		...
		$this->form_validation->set_rules('pemohon_email', 'Email Pemohon', 'trim|required|valid_email|is_unique[aauth_users.email]');
		...
		$this->form_validation->set_rules('pemohon_rt', 'RT', 'trim|required|max_length[5]');
		$this->form_validation->set_rules('pemohon_rw', 'RW', 'trim|required|max_length[5]');
		...
		$password = '@Semarang@143!';

		if ($this->form_validation->run()) {
			$kode = 'user'.nomor_acak(11);

			$data_user = $this->model_pemohon->query_pemohon()->result();

			if (count($data_user) > 0) {
				foreach ($data_user as $user) {
					if ($user->pemohon_kode == $kode) {
						$kode_user = 'user'.nomor_acak(11);
					}else{
						$kode_user = $kode;
					}
				}
			...
			$data_user = [
				'full_name' => $this->input->post('pemohon_nama'),
			];

			$save_user = $this->aauth->create_user($this->input->post('pemohon_email'), $password, $kode_user, $data_user);
			...
			if ($save_user) {
				...
				$save_data = [
					...
					'pemohon_created_at' 	=> date('Y-m-d H:i:s'),
				...
				$this->data['message'] = $this->aauth->print_errors();
			}
		} else {
			$this->data['success'] = false;
	...
	*/
	public function edit_save($id) {
		...
		$this->form_validation->set_rules('pemohon_nama', 'NamaPemohon', 'trim|required|max_length[65]');
		$this->form_validation->set_rules('pemohon_jenkel', 'Jenis Kelamin', 'trim|required');
		$this->form_validation->set_rules('pemohon_alamat', 'Alamat Pemohon', 'trim|required');
		$this->form_validation->set_rules('pemohon_rt', 'RT', 'trim|required|max_length[5]');
		$this->form_validation->set_rules('pemohon_rw', 'RW', 'trim|required|max_length[5]');
	...
	*/
	private function _remove($id) {
		...
		$this->db->delete('aauth_users', ['id' => $pemohon->user_id]);
		$this->db->delete('aauth_user_to_group', ['user_id' => $pemohon->user_id]);
