		...
		<div class="col-md-12">
			...
			?>
				<div class="box-header">
					<h5 class="box-title"><?= cclang('update', ['Menara']); ?></h5>
				</div>
				<div class="box-body">
					<div class="row">
						<div class="col-md-6">
							<div class="form-group group-pemohon-id">
								...
								<select class="form-control chosen chosen-select-deselect" name="pemohon_id" id="pemohon_id" data-placeholder="Select Pemohon">
									<option value=""></option>
									<?php foreach (db_get_all_data('pemohon') as $row): ?>
							...
							<div class="form-group group-tipesite-id">
								...
								<select class="form-control chosen chosen-select-deselect" name="tipesite_id" id="tipesite_id" data-placeholder="Select Tipe Site">
									<option value=""></option>
									<?php foreach (db_get_all_data('tipe_site') as $row): ?>
							...
							<div class="form-group group-operator-id">
								...
								<select class="form-control chosen chosen-select" name="operator_id[]" id="operator_id" data-placeholder="Select Operator" multiple>
									<option value=""></option>
									<?php foreach (db_get_all_data('operator') as $row): ?>
									<option
										<?= in_array($row->operator_id, explode(',', $menara->operator_id)) ? 'selected' : ''; ?> value="<?= $row->operator_id ?>"><?= $row->operator_nama; ?></option>
									<?php endforeach; ?>
								</select>
								<small class="info help-block"></small>
							</div>
							...
							<div class="form-group group-menara-rt">
								...
								<small class="info help-block"><b>Input Menara Rt</b> Max Length : 5.</small>
							</div>
							...
							<div class="form-group group-menara-rw">
								...
								<small class="info help-block"><b>Input Menara Rw</b> Max Length : 5.</small>
							</div>
							...
							<div class="row">
								<div class="col-md-12">
									<div class="form-group group-menara-latitude">
										...
										<input type="text" class="form-control" name="menara_lat_lng" id="menara_lat_lng" placeholder="Latitude, Longitude" value="<?= set_value('menara_latitude', $menara->menara_latitude.', '.$menara->menara_longitude); ?>">
										<small class="info help-block">Copy and paste kordinat peta atau tentukan lewat peta dibawah</small>
									</div>
								</div>
							</div>
							<div class="row">
								...
								<div class="col-md-12">
									...
									<div class="form-group group-kawasan-id">
										...
										<select class="form-control chosen chosen-select-deselect" name="kawasan_id" id="kawasan_id" data-placeholder="Select Kawasan">
											<option value=""></option>
											<?php foreach (db_get_all_data('kawasan') as $row): ?>
									...
									<div class="form-group group-menara-tgl-imb">
										...
										<div class="input-group date col-sm-12">
											<input type="text" class="form-control pull-right datepicker" name="menara_tgl_imb" placeholder="" id="menara_tgl_imb" value="<?= set_value('menara_tgl_imb', !empty($menara->menara_tgl_imb) ? $menara->menara_tgl_imb : ''); ?>">
										</div>
										<small class="info help-block"></small>
									</div>
								</div>
...
<script type="text/javascript">
	$(document).ready(function () {
		...
		markerDraggable.on('dragend', function (e) {
			$('#menara_lat_lng').val(markerDraggable.getLatLng().lat+', '+markerDraggable.getLatLng().lng);
		});

		$("#menara_lat_lng").change(function() {
			var lat_lng 		= $('#menara_lat_lng').val();
			var lat_lng_replace = lat_lng.replace(', ', ',');
			var position 		= lat_lng_replace.split(',');

			// console.log({Lat: Number(position[0]), Lng: Number(position[1])});

			pin = L.marker(position,{ riseOnHover:true, draggable:true });
		...
		$('#menara_menara_file_berkas_galery').fineUploader({
			...
			session: {
				endpoint: BASE_URL +
					'administrator/menara/get_menara_file_berkas_file/<?= $menara->menara_id; ?>',
		...
		$('#menara_menara_image_galery').fineUploader({
			...
			session: {
				endpoint: BASE_URL +
					'administrator/menara/get_menara_image_file/<?= $menara->menara_id; ?>',
				refreshOnRequest: true
			},
			validation: {
