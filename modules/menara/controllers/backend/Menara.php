...
class Menara extends Admin {
	...
	*/
	public function add_save() {
		if (!$this->is_allowed('menara_add', false)) {
			...
			$expl_lat_lng = explode(',', str_replace(' ', '', $this->input->post('menara_lat_lng')));
			echo json_encode($expl_lat_lng);
			exit;
		...
		$this->form_validation->set_rules('menara_lat_lng', 'Longitude & Latitude', 'trim|required|max_length[255]');
		...
		if ($this->form_validation->run()) {
			...
			$save_data = [
				...
				'menara_latitude' 		=> $expl_lat_lng[0],
				'menara_longitude' 		=> $expl_lat_lng[1],
				...
				'menara_tgl_imb' 		=> !empty($this->input->post('menara_tgl_imb')) ? $this->input->post('menara_tgl_imb') : NULL,
	...
	*/
	public function view($id) {
		...
		$this->data = [
			'zona' 		=> $zonalatLng,
			'menara' 	=> $this->model_menara->join_avaiable()->filter_avaiable()->find($id),
		];
